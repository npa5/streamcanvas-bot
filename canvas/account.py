...
def auth(accounts, channel):
    ...
    for acc in accounts:
        ...
        token[acc] = requests.post("https://gql.twitch.tv/gql", json=body, headers=headers).json()["data"]["extensionLinkUser"]["token"]["jwt"] # this ACTUALLY gets the extension auth token, i have no idea why twitch requires another token just to get this but thats just twitch ig 
